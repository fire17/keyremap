...
[![tests](https://img.shields.io/badge/tests-135%20passing-5ddba4)](tests/test_keyremap.py)
...
- **The same firmware quirk must be handled on every platform.** This keypad toggles NumLock
  ...
  never touched). The same reasoning applies to Linux, where evdev likewise reports what the
  device actually sends — so the numpad twin is accepted there too, proven against a real
  kernel device in CI.
- **A keypad's nav keys are often numpad keys in disguise.** Windows converts them
  ...
  which costs nothing (it is already scoped to one device) and turns a silent no-match into
  a working key.
